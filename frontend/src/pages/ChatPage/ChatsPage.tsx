...
import CreateGroupChatModal from "../../components/features/chat/CreateGroupChatModal/CreateGroupChatModal";
import ImageModal from "../../components/features/chat/ImageModal/ImageModal";
import MessageContextMenu from "../../components/features/chat/MessageContextMenu/MessageContextMenu";
import ChatProfileModal from "../../components/features/chat/ChatProfileModal/ChatProfileModal";
import ForwardMessageModal from "../../components/features/chat/ForwardMessageModal/ForwardMessageModal";
import AlertModal from "../../components/shared/ui/AlertModal/AlertModal";
import { layoutImages } from "../../components/features/chat/ImageModal/imageLayout";
import "./chats.css";
      ...
      if (event.type === "chat.deleted") {
        ...
        if (activeChatIdRef.current === deletedChatId) {
          ...
          setAlertModal({
            isOpen: true,
            title: "Chat deleted",
            message: `Chat "${chatTitle}" has been deleted.`,
            variant: "warning",
          });
        }
      ...
      case 'copy-image':
        if (attachment) {
          try {
            ...
            alert('Failed to copy image. Try downloading it.');
          }
        }
        break;
          ...
          {loading ? (
            ...
            <>
              <div className="chat-contact-list">
                {chats.map((chat) => (
                  ...
                  >
                    ...
                    {chat.is_new && (
                      <span className="chat-contact-card__new-badge" title="New chat">
                        NEW
                      </span>
                    )}
                  </button>
              ...
              {contactsWithoutChats.length > 0 && (
                <>
                  <div className="chat-sidebar__subtitle">Contacts</div>
              ...
              {chats.length === 0 && contactsWithoutChats.length === 0 && (
                <div className="chat-empty">No chats or contacts yet</div>
              )}
            </>
          )}
        </div>
        ...
        {activeChatId || activeRecipientId || messages.length > 0 ? (
          <>
              ...
              >
                ...
                <div className="chat-main__status">
                  {chatLoading
                    ...
                    : activeChatId
                      ? "Open"
                      : "New chat will be created on first message"}
                </div>
              </div>
            ...
            >
              ...
              {olderLoading && (
                <div className="chat-main__older-loading">Loading older messages...</div>
              )}

              {chatLoading ? (
                <div className="chat-main__placeholder">Loading history...</div>
              ) : messages.length === 0 ? (
                          ...
                          >
                            {message.forwarded_from && (
                              <div className="chat-message__forwarded-label">
                                Forwarded from {message.forwarded_from.sender.full_name}
                              </div>
                            )}
                            {message.reply_to && (
                              <div
                            ...
                            <span className="chat-message__meta">
                              <span className="chat-message__time">
                                ...
                                {message.edited_at && (
                                  <span className="chat-message__edited" title={`Edited ${new Date(
                                    message.edited_at).toLocaleString('en-US')}`}>
                                    {' '}edited
                                  </span>
                                )}
                              </span>
              ...
              {newerLoading && (
                <div className="chat-main__older-loading">Loading newer messages...</div>
              )}
            </div>
            ...
            <div className="chat-main__footer">
              {showScrollToBottom && (
                <button
                  ...
                  title={unreadCount > 0 ? "Unread messages" : "Scroll to bottom"}
                >
                  <span className="chat-scroll-to-bottom__icon"><RiArrowDownLine size={22} /></span>
                  {unreadCount > 0 && (
              ...
              {editingMessage && (
                <div className="chat-edit-preview">
                  <div className="chat-edit-preview__label">Editing</div>
                  <div
              ...
              {replyingToMessage && (
                <div className="chat-reply-preview">
                  <div className="chat-reply-preview__label">Reply</div>
                  <div
              ...
              {forwardingMessage && (
                <div className="chat-forward-preview">
                  <div className="chat-forward-preview__label">Forwarding</div>
              ...
              <div className="chat-main__composer">
                ...
                >
                  {editingMessage ? <RiCheckLine size={20} /> : <RiSendPlaneFill size={18} />}
                </button>
              </div>
            </div>
          </>
